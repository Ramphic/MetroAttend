    ...
    <AdminShell nav={nav}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-muted text-sm mt-0.5">Real-time attendance logs, status overrides, and verification verification</p>
        </div>
        <button 
          onClick={() => exportRecordsToCSV(filtered, `attendance_${selectedDate}.csv`)}
          className="flex items-center gap-2 bg-navy text-white px-4 py-2.5 rounded-xl font-display font-semibold text-xs hover:bg-navy-dark transition-all shadow-xs"
        >
